...
exp:一个包含5大函数的类
	...
	public:
		...
		IntCell( const IntCell & src )
			// 拷贝构造函数，处理左值
			{ storeValue = new int( *src.storeValue ); }  //*修饰storeValue而非src，相当于*(src.storeValue)，解析地址获得值
...
exp: 调用拷贝构造函数：
	...
	Cell cp2 = ob; // 虽然使用=，但仍调用Cell类的拷贝构造函数

	Cls realArg;
	Cls func(Cls par){return par};
	Cls retObj = func(realArg); //参数与返回值对象会调用自身类的拷贝构造函数
...
NOTE：若只定义拷贝构造函数，而无其他构造函数，则无法创建这个类的对象(若本体无法创建，那么也无法创建复制体）
...
1. 源对象oCe有成员mCell，是一个指向数组的指针，
1. 先将mCell指针赋值给新对象的成员nCe.mCell，
1. nCe.mCell获得了这块内存的控制，
1. oCe.mCell指针被设为空指针，
1. 这样oCe的析构函数运行时（会销毁oCe.mCell），nCe.mCell不受影响

TODO：move(oCe)会不会导致源对象整个为Null

#### noexcept

> 告诉编译器，它不会抛出任何异常，保证兼容标准库
...
数据成员为指针，默认跟int，double等一样处理（直接对值复制）：导致内存泄漏或错误

TODO：测试move对象的销毁情况

1. 拷贝构造函数产生问题：指针未创建，用其他指针指向的地址值进行初始化，新旧对象都拥有此地址的所有权
2. 拷贝赋值运算符产生问题：指针指向的指针已创建，地址a直接修改为地址b，导致无指针指向a，但它又未被销毁，导致内存泄漏，且新旧对象都拥有b的所有权
...
exp: 错误（默认）的指针处理1
	...
	public:
		Pt() = delete;
		Pt(string name):mElem(name){
			mPpt = new vector<int>{3,4,6,7};
		...
		Pt(const Pt& src){
			...
			// 然后2个指针指向同一个地制值
			mPpt = src.mPpt;
			cout << name << " copied construct" << endl;
		}
		Pt& operator=(const Pt& rhs){
			cout << name << " operator= assignment" << endl;
			if (this == &rhs){
				return *this;
			}
			// 此时mPpt已初始化
			// 指针直接指向其他地址，导致原先指向的地址被遗弃，导致内存泄漏
			mPpt = rhs.mPpt;
			return *this;
		}
		vector<int> *mPpt;
	...
	int main(){
		...
		Pt second = origin;
		second.mPpt[1] = 65;
		cout << "origin value: " << origin.mPpt[1] << endl;
		Pt third("third");
		third = origin;
		third.mPpt[1] = 32;
		cout << "origin value: " << origin.mPpt[1] << endl;
...
特性：在main(){}执行完成之后，再调用析构函数(mechanism)

1. 类的静态成员：一些通常不变或只对类有意义的数据，只需在类中存储一份，而不是所有实例中都包含。 
    - 必须先初始化，因为实例中最多只能修改它的值而不能创建它
    - 默认初始化为0或nullptr。
    - static类成员需要在类定义的cpp文件中分配内存（初始化）
    - 实例对象调用类的静态成员时，实际上调用的是Cls::staticMember
2. 类的静态方法：方法只用于类对象而非实例对象，不访问特定对象（指非静态的类数据成员）的信息。
    - 静态方法相当于在类之外（指全局环境）定义的普通函数，但却能访问此类的private和protected静态数据成员
    - 某个实例对象调用静态方法时，只会访问这个对象的静态数据成员
3. 普通函数中的静态变量：记住某个函数是否执行了特定的初始化操作，但是可能令人迷惑。不建议使用。
...
形式：
    ...
    Cls{
        ...
        static type func(type param){}
    };
	type Cls::staticMember = value;  //此处不需要再加static
...
> reference: [cv (const and volatile) type qualifiers](https://en.cppreference.com/w/cpp/language/cv)
...
exp:在类中声明一个使用const的方法
	...
	// 其实应为 int const，但习惯上写为 const int 
	`const int * const Method3(const int * const & refPointer) const;`
...
NOTE: const对象, 如`const ClsName ins(10);`, 只能调用有第5种const修饰的类方法。

	#include <iostream>
	using namespace std;

	const int * const Method3(const int * const refPointer, int ano)
	{
		TODO：测试，再测试ano是pointer的情况
		ano += *refPointer;
		return &ano;
	}
	int main(){
		...
		int aa = 14;
		// int * reff = &aa;
		const int* const ref = &ace;
		const int * const val = Method3(ref,aa)
	}

#### const与引用：

对于引用`&`，不需要用const修饰，因为引用指向的对象不变(const)。
...
NOTE：类数据成员若为const引用，则只能调用此类数据成员的const方法

NOTE：不要将析构函数设置为const，否则无法正确地销毁const对象

### constexpr(常量表达式)

- const 运行时求值（exe程序运行时）
- constexpr 编译期间求值（compiler编译时），编译后就固定了，修饰函数则不允许有任何副作用(mechanism)
...
- constexpr int foo(){return 10;}  // 函数无参数
...
作用：成员方法"逻辑"上应该是const，但它实际改变了数据成员，将这些数据用mutable修饰，避免错误，保证其逻辑上的const。

exp：const成员方法，其中的mCounter却需要发生变化（其他数据成员不变），使用mutable修饰并初始化
